...
public record CreateOrUpdateCommpanyCmd
{
    ...
    public class CreateCommpanyCmdValidator : AbstractValidator<CreateOrUpdateCommpanyCmd>
    {
        ...
        public CreateCommpanyCmdValidator(ICompanyApp companyApp)
        {
            ...
            RuleFor(x => x.AdjustmentPriceValue).Cascade(CascadeMode.Stop).NotNull().When(x => x.AdjustmentPriceType.HasValue)
                .WithMessage("调价类型设置时需要提供调价值")
                .GreaterThanOrEqualTo(0).When(x => x.AdjustmentPriceType.HasValue)
                .WithMessage("调价值不能小于0");
